...
# terraform-aws-superblocks module block with two new arguments:
#
...
module "superblocks_opa1" {
  ...
  # Must match the agent_tags declared in native_db_prereqs above.
  superblocks_agent_tags = "profile:nonprod,profile:production"

  # Wire the lifecycle worker role as the ECS task role so the container can
  # assume it for OpenTofu S3 state access and RDS provisioning.
  superblocks_agent_role_arn = module.native_db_prereqs.agents["opa1"].lifecycle_worker_role_arn

  # Pass all native DB env vars generated by modules/native-db into the container.
  superblocks_agent_environment_variables = module.native_db_opa1.ecs_env_vars
